function update() {
  ...
  ["top", "right", "bottom", "left"].forEach(function (value) {
    labelCSS +=
      bubbleCSS('label[for="side-' + value + '"]', {
        side: value,
        triangle: triangle,
        size: 8,
        color: "yellowgreen",
  ...
  ["symmetrical", "right", "left"].forEach(function (value) {
    labelCSS +=
      bubbleCSS('label[for="triangle-' + value + '"]', {
        side: side,
        triangle: value,
        size: 8,
        color: "deeppink",
...
function bubbleCSS(selector, settings) {
  ...
  propsBefore["border"] =
    getLength(settings.size, settings.ems) + " solid transparent";
  ...
  if (triangle != "symmetrical") {
    propsBefore[
      "border-" +
        (isHorizontal ? triangle : triangle == "right" ? "top" : "bottom")
    ] = "0";
  }

  propsBefore["margin-" + offset] = getLength(
    -settings.size / (triangle == "symmetrical" ? 1 : 2),
    settings.ems
  ); // to center it
...
$$("input").forEach(function (input) {
  input[
    input.type == "radio" || input.type == "checkbox" ? "onclick" : "oninput"
  ] = update;
});
